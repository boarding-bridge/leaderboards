...
const I18N = {
  ja: {
    ...
    roiNotePre: "●大会開始前のエントリー確認表示です。Depositはこれまでの累計入金額です（参考表示）。<br>●リワード対象の判定は大会期間中の入金 200 USDC以上で行われます（大会開始後に期間中の入金で判定されます）。<br>●ランキング・ROI等の数値は大会開始後に表示されます。",
  ...
  en: {
    ...
    roiNotePre: "●Pre-competition entry check. Deposit shows your total deposits so far (for reference).<br>●Reward eligibility is determined by deposits of 200 USDC or more during the competition period (evaluated after the competition starts).<br>●Rankings, ROI and other stats will appear once the competition starts.",
    volTotalLabel: "Total Volume: ",
